...
class RenderSpace extends RenderBox {
  ...
  void performLayout() {
    if (parent is RenderFlex) {
      final RenderFlex flexParent = parent as RenderFlex;
      if (flexParent.direction == Axis.horizontal) {
        size = constraints.constrain(Size(mainAxisExtent, 0));
      } else {
        size = constraints.constrain(Size(0, mainAxisExtent));
      }
    } else {
      throw FlutterError('Space widget must be a child of a Flex widget.');
    }
  }
}
